...
def main(argv=None):
    ...
    win = glfw.create_window(cfg.width, cfg.height, "Ghoul Fluids (Split View)", None, None)
    ...
    try:
        while not glfw.window_should_close(win):
            ...
            if glfw.get_key(win,glfw.KEY_SPACE)==glfw.PRESS: running = not running; time.sleep(0.12)
            if glfw.get_key(win,glfw.KEY_C)==glfw.PRESS:
                for fbo in (sim.fbo_vel_a, sim.fbo_vel_b, sim.fbo_dye_a, sim.fbo_dye_b,
                            sim.fbo_prs, sim.fbo_prs_b, sim.fbo_div, sim.fbo_curl):
                    fbo.use(); ctx.clear(0,0,0,1)
                ctx.screen.use(); time.sleep(0.12)
            if glfw.get_key(win,glfw.KEY_LEFT_BRACKET)==glfw.PRESS:
                sim.prog_vort['eps'].value = max(0.0, sim.prog_vort['eps'].value - 0.1); time.sleep(0.05)
            if glfw.get_key(win,glfw.KEY_RIGHT_BRACKET)==glfw.PRESS:
                sim.prog_vort['eps'].value = min(5.0, sim.prog_vort['eps'].value + 0.1); time.sleep(0.05)
            if glfw.get_key(win,glfw.KEY_P)==glfw.PRESS:
                cfg.palette_on = 1 - cfg.palette_on; time.sleep(0.12)
            if glfw.get_key(win,glfw.KEY_V)==glfw.PRESS:
                split_view = not split_view; time.sleep(0.12)
            ...
            if running:
                ...
                if cfg.use_fallback_orbit and not have_mask:
                    ...
                    for i in range(cfg.micro_splats_per_frame):
                        ang = 2*math.pi * (i / cfg.micro_splats_per_frame + 0.1 * math.sin(t*0.4))
                        ...
                        k = (i / cfg.micro_splats_per_frame + 0.2 * math.sin(t*0.7)) % 1.0
                        col = (orange * (1.0 - k) + purple * k) * cfg.micro_splat_strength

                        sim.fbo_dye_b.use()
                        sim.dye_a.use(location=0)
                        sim.prog_splat['field'].value = 0
                        sim.prog_splat['point'].value = (float(u), float(v))
                        sim.prog_splat['value'].value = (float(col[0]), float(col[1]), float(col[2]))
