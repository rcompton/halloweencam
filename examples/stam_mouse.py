...
def main():
    ...
    win = glfw.create_window(
        WIDTH, HEIGHT, "Step 4: Mouse Stable Fluids (min)", None, None
    )
    ...
    while not glfw.window_should_close(win):
        ...
        if glfw.get_key(win, glfw.KEY_LEFT_BRACKET) == glfw.PRESS:
            val = max(0.0, prog_vort["eps"].value - 0.1)
            prog_vort["eps"].value = val
            time.sleep(0.05)
        if glfw.get_key(win, glfw.KEY_RIGHT_BRACKET) == glfw.PRESS:
            val = min(5.0, prog_vort["eps"].value + 0.1)
            prog_vort["eps"].value = val
            time.sleep(0.05)
        ...
        mforce = (
            (mvel[0] * MOUSE_VEL_SCALE, mvel[1] * MOUSE_VEL_SCALE)
            if mouse.down
            else (0.0, 0.0)
        )
