...
def test_mask_force_changes_velocity(ctx, small_cfg):
    ...
    # velocity initially near-zero; upload a ring mask to create edges
...
def test_advects_dye_even_without_mask(ctx, small_cfg):
    sim = FluidSim(ctx, small_cfg)
    # run a few steps without mask; with zero velocity and dissipation=1.0,
    # dye stays same. This is a stability smoke test (no exceptions).
    for _ in range(3):
